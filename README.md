...
- 道路ごとに選ばれる人工道・自然道と、それぞれ3種類の素材パレット
- 人工道: 泥レンガ系、磨かれた安山岩系、石レンガ系
- 自然道: 粗い土系、丸石系、土の道系
- 水面にバイオーム別の板材を置く最小限の橋
- 連続水域の長さに応じた幅広の橋床、両側の欄干、浅い川の木製橋脚
- 橋の両端にランタン付きの親柱
- 1ブロック段差に、人工道では階段、自然道ではハーフブロックを設置
- 最大調整量を守る切土・盛土で、2ブロック以上の高低差を緩和
- 人工道の吊りランタン付き街灯と、自然道の松明付き道標
...
- `allowArtificialRoads`: 人工道を生成候補に含めるか
- `allowNaturalRoads`: 自然道を生成候補に含めるか
- `placeRoadDecorations`: 街灯と道標を設置するか
- `decorationSpacing`: 街灯・道標を置く中心線上のおおよその間隔
- `smoothRoadSlopes`: 1ブロック段差を階段・ハーフブロックで補うか
- `gradeRoadTerrain`: 急な道路で切土・盛土を行うか
- `maximumTerrainAdjustment`: 地表を上下させる最大ブロック数
- `minimumBridgeSpan`: 橋として整形する連続水域の最小長
- `bridgePillarSpacing`: 橋脚同士のおおよその間隔
- `maximumBridgePillarDepth`: 橋脚を設置する水深の上限
...
- 街灯と道標は最小デザインのみで、距離標識や複数の装飾デザインはまだありません。
- 橋は直線的な基本デザインです。設定上限より深い水域では橋脚を省略します。
- 切土・盛土の最大調整量を超える極端な崖は、完全には平滑化しません。
